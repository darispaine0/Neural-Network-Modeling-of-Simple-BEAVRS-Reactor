...
For local environment setup:
    ...
    3. export OPENMC_CROSS_SECTIONS=/path/to/cross_sections.xml
"""
...
class DataGenConfig:
    ...
    INPUT_TEMPS_FILE = "input_temps300k.npy"
    OUTPUT_SOURCE_FILE = "output_source.npy"
    OUTPUT_FM_RAW_FILE = "output_fm_raw.npy"
...
def extract_results(statepoint_file='statepoint.100.h5'):
    ...
    Returns:
        dict: Contains keff, source, fm_raw, and uncertainties
    """
    ...
    results = {
        ...
        'fm_raw_mean': fm_tally.mean.squeeze(),
        'fm_raw_std': fm_tally.std_dev.squeeze()
    }
...
def build_normalized_fission_matrix(fm_raw, n_cells):
    """
    ...
    Args:
        fm_raw: Raw fission matrix data (flattened, length n_cells²)
    ...
    for i in range(n_cells):
        for j in range(n_cells):
            idx = j * n_cells + i
            if fm_raw[idx] > 0:
                column_sum = np.sum(fm_raw[j*n_cells:(j+1)*n_cells])
                if column_sum > 0:
                    fm[i, j] = fm_raw[idx] / column_sum * np.sum(fm_raw)
...
class DataCollector:
    ...
    def add_run(self, temps_tuple, results, fm_normalized, uncertainties, validation_metrics):
        ...
        self.fm_raw_data.append(results['fm_raw_mean'])
    ...
    def save_to_directory(self, output_dir, config):
        ...
        np.save(output_dir / config.OUTPUT_FM_RAW_FILE, np.array(self.fm_raw_data, dtype=object))
...
def main():
    ...
    for run_idx in range(args.runs):
        ...
        fm_normalized = build_normalized_fission_matrix(results['fm_raw_mean'], config.N_CELLS)
